...
def aeronet_pandas_generator(aerofile,aerofilt_dir):
    ...
    for day in dates:
        ...
        dt.append(pan.datetime(yr,mn,dy,hr))

    ind = pan.DatetimeIndex(dt)
    ...
    for n in range(0,len(dt)):
        try:
            newdict['Diameters'].append(np.array(numdist_diameters))
        except KeyError:
            newdict['Diameters'] = np.array(numdist_diameters)
        try:
            newdict['Numdist'].append(np.array(temp_dist[n,:], dtype='float16'))
        except KeyError:
            newdict['Numdist'] = np.array(temp_dist[n,:], dtype='float16')
